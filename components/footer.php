<footer class="flex col aic jcc">
            <div class="footer-content grid col-12-8-4">
                <article class="footer-text flex col jcsb">
                    <div class="flex col gap-16">
                        <h4>let's talk!</h4>
                        <p>like my work? have a few questions?
                        <br>feel free to reach out, i’m always looking to collaborate!</p>
                    </div>
                    <a href="mailto:hello@paularies.ca" class="email flex row aic gap-8" tabindex="0">
                        <img src="./icons/ui/black-mail.svg" alt="Email icon." draggable="false">
                        <p>info@paularies.ca</p>
                    </a>
                </article>
                <div class="footer-links flex col gap-8">
                    <a href="https://www.linkedin.com/in/paularies/" target="_blank">linkedin</a>
                    <a href="https://github.com/plries" target="_blank">github</a>
                    <a href="https://www.instagram.com/plries/" target="_blank">instagram</a>
                </div>
                <a href="#" class="footer-logo">
                    <img src="./icons/ui/logo-big.svg" alt="Paul Aries Logo." draggable="false">
                </a>
            </div>         
        <div class="footer-icons flex row aic jcc gap-32">
            <img src="./icons/ui/black-tree.svg" alt="Tree icon." class="footer-icon" draggable="false">
            <img src="./icons/ui/black-target.svg" alt="Target icon." class="footer-icon" id="target" draggable="false">
            <img src="./icons/ui/black-star.svg" alt="Star icon." class="footer-icon" draggable="false">
        </div>
    </footer>